...
class TestParetoExpDistributionDataset:
    ...
    def test_input_output_length_distribution(self, dataset, tokenizer, random_seed, input_mean, output_mean, num_samples):
        ...
        assert np.all(input_lengths <= dataset.max_length), "All input lengths should be <= max_length"
        ...
        D, p_value = stats.kstest(input_lengths, 'pareto', args=(dataset.pareto_a, 0, pareto_b))
        ...
        assert np.all(output_lengths <= dataset.max_length), "All output lengths should be <= max_length"
